...
from mdt_msgs.msg import DrixCommand
...
class Drix_data(object): # class to handle the data from the rosbags
    ...
    def __init__(self,L_bags):
        ...
        self.list_topics = ['/gps', '/kongsberg_2040/kmstatus','/drix_status','/telemetry2','/d_phins/aipov','/mothership_gps','/rc_command',
        '/gpu_state','/trimmer_status','/d_iridium/iridium_status','/autopilot_node/ixblue_autopilot/autopilot_output',
        '/bridge_comm_slave/network_info','/cc_bridge_comm_slave/network_info','/command','/rc_feedback','/diagnostics']
    ...
    def result_path_setup(self): # create all the folders needed to store the data
        ...
        if not os.path.exists(self.result_path):
            ...
            shutil.rmtree(self.result_path) # clean the directory in order not to have any disruptive past data (diagnotics data)
            os.makedirs(self.result_path)
    ...
    def rosbag2pd(self, L_bags):
        ...
        for bagfile in L_bags:
            ...
            dic_drix_status = {'Time':[],'gasolineLevel_percent':[],'drix_mode':[],'emergency_mode':[],"drix_clutch":[],"keel_state":[],'shutdown_requested':[],'thruster_RPM':[]}
            ...
            dic_iridium_status = {'Time':[],'is_iridium_link_ok':[],"signal_strength": [],"registration_status": [],"last_mo_msg_sequence_number": [],"failed_transaction_percent": []}
        ...
        if len(cc_bridge_comm_slave_pd) > 0:
            ...
            print3000("Cc_Bridge comm_slave data imported : " + str(len(cc_bridge_comm_slave_pd)) + '/' + str(len(L_bags)))
        else:
