...
class recup_data(object):
	...
	def set_up_splitter(self): # set up the sub-parts border
		...
		OFF_SET = datetime.strptime(OFF_set, '%d-%m-%Y-%H-%M-%S') # datetime object
		...
		if self.date_ini >= OFF_SET:  # the data start after the offset 
			A = OFF_SET + timedelta(hours=int(l[0]), minutes=int(l[1]), seconds=int(l[2]))
	...
	def collect_data(self): # Recover the data from the compressed folder
		...
		if self.folder_zipped:
			...
			if not os.path.exists(self.result_folder):
				...
				shutil.rmtree(self.result_folder) # clean the directory in order not to have any disruptive past data (diagnotics data)
				os.makedirs(self.result_folder)
		...
		# ~ ~ All others ~ ~
		print('rebuild all the others data')
		...
		for key in l: # in order to not consider self.gps  
	...
	def MSG_gps(self):
		...
		if 'gps' in list(self.data.keys()) and self.data['gps'] is not None:
			...
			if "path_following" in self.data["pie_chart"]['Name'].tolist():
				path_following_dist = self.data["pie_chart"][self.data["pie_chart"]['Name'] == 'path_following']['L_dist'].item()
				path_following_speed = self.data["pie_chart"][self.data["pie_chart"]['Name'] == 'path_following']['L_speed'].item()
				path_following_dt = self.data["pie_chart"][self.data["pie_chart"]['Name'] == 'path_following']['list_dt_str'].item()
			...
			return({"global_dist":"DataNotFound","global_speed":"DataNotFound","global_knots":"DataNotFound", "path_following_dist" : "DataNotFound", "path_following_speed" : "DataNotFound", 'path_following_dt' : "DataNotFound"})
	...
	def MSG_phins(self):
		...
		if 'pitchDeg' in list(self.data.keys()) and self.data['pitchDeg'] is not None:
			...
			return({"pitch_min" : "DataNotFound", "pitch_mean" : "DataNotFound", "pitch_max" : "DataNotFound", "roll_min" : "DataNotFound","roll_mean" : "DataNotFound", "roll_max" : "DataNotFound"})
	...
	def read_data_from_csv_files(self): # recover the data from csv files generated with the above function (data already decompressed)
		...
		self.L_diags_name = [name.split('.')[0] for name in list_csv if name.split('.')[0] not in list_data_name] # determine the names of the diagnotics topic by deducing from the label.xlsx file
			...
			if df['Time'][k] > self.border_sub_parts[i]:
				...
				while(df['Time'][k] > self.border_sub_parts[i]):
					...
					if i >= len(self.border_sub_parts):
						print("Error border_sub_parts not valid")
		...
		# (when you remove a point, it cancels the line with the next point, so we add fake points in the parts border in order to not impact the curve) 
			...
			if dd:
				...
				if index > 0 and 'y' in df.columns.tolist():
					...
					if isinstance(df.iloc[index-1]["y"],type(np.bool_(True))): # because the addition doesn't work with bool variable
